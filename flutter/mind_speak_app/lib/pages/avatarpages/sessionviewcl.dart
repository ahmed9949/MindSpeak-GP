...
// Update animation control to be more robust
  void _updateAnimationForSpeechState(String state) {
    ...
    switch (state) {
      case "waitingStart":
        _lockAnimation(true);
        _playAnimation("newtalk"); // Use talk animation for waiting phrase
        break;
      ...
      case "mainStart":
        _lockAnimation(true);
        // Force re-triggering the animation even if it's already playing
        // to ensure it's synchronized
        controller.playAnimation(animationName: "newtalk");
        _currentAnimation = "newtalk";
        _lastAnimationChange = DateTime.now();
        print("🎭 Forced newtalk animation for main speech");
        break;

      case "mainEnd":
        // Short delay before unlocking to prevent immediate idle animation
  ...
  Future<void> _speak(String text, {bool isConversation = false}) async {
    if (!_isSpeaking && mounted) {
      try {
        ...
        // Use event-based animation control instead of direct calls
        // (handlers will update animations)
  ...
  void _playAnimation(String animation) {
    try {
      // Prevent animation spam (minimum 300ms between animation changes)
      final now = DateTime.now();
      if (now.difference(_lastAnimationChange).inMilliseconds < 300 &&
          animation == _currentAnimation) {
        print("🎭 Animation skipped (duplicate): $animation");
        return;
      }

      // If animation is locked, only allow specific priority animations
      if (_animationLocked &&
          animation != "newtalk" &&
          !animation.contains("error")) {
        print("🔒 Animation locked, skipping: $animation");
        return;
      }

      print("🎭 Playing animation: $animation (previous: $_currentAnimation)");

      // Debug info to help diagnose timing and state issues
      print("  🔍 _isSpeaking: $_isSpeaking");
      print("  🔍 _isListening: $_isListening");
      print("  🔍 TTS isPlaying: ${_ttsService.isPlaying}");
      print("  🔍 TTS isMainSpeechActive: ${_ttsService.isMainSpeechActive}");
  ...
  void _lockAnimation(bool lock) {
    ...
    print("🔒 Animation ${lock ? 'locked' : 'unlocked'}");
  }
