...
class ChatGptTtsService {
  final String _apiKey = dotenv.env['OPEN_AI_API_KEY']!;
  String _voice = 'nova'; // Make this modifiable
  final String _model = 'gpt-4o-mini-tts';
  ...
  ChatGptTtsService({String? voice}) {
    // Set voice if provided in constructor
    if (voice != null && voice.isNotEmpty) {
      _voice = voice;
    }
  ...
  // Allow voice to be changed
  void setVoice(String voice) {
    if (_voice != voice) {
      print("🎤 Changing voice from $_voice to $voice");
      _voice = voice;
      // Clear cache to ensure new voice is used
      _audioCache.clear();
    }
  }
  ...
  Future<void> initialize() async {
    ...
    print("✅ Common TTS phrases preloaded with voice: $_voice");
  }
  ...
  Future<void> _prefetchAudio(String text) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            "❌ Failed to cache \"$text\": ${response.statusCode} - ${response.body}");
      }
    } catch (e) {
      print("❌ Error caching \"$text\": $e");
    }
  ...
  Future<void> speak(String text) async {
    try {
      // Reset retry counter on new speak request
      _retryCount = 0;
      return await _speakWithRetry(text);
    } catch (e) {
      _isPlaying = false;
      print("❌ TTS speak error: $e");
      // Don't throw so UI can continue
    }
  }
    ...
    try {
      ...
      if (_audioCache.containsKey(text)) {
        ...
        try {
          ...
          print("⚠️ Error playing cached audio, trying to regenerate: $e");
          // Continue to API call if cache playback fails
        }
      }
      ...
      if (_retryCount < _maxRetries) {
        _retryCount++;
        print(
            "🔄 Retrying TTS playback (attempt ${_retryCount}/${_maxRetries})");
        await Future.delayed(const Duration(milliseconds: 500));
